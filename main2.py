...
# ==========================================
# УСИЛЕННЫЙ АСИНХРОННЫЙ КОНВЕЙЕР
# ==========================================
def run_heavy_pipeline(job_id: str, video_path: str):
    ...
    while cap.isOpened():
        ...
        if frame_idx % sample_rate == 0:
            ...
            # Включаем чуть более чувствительный порог conf > 0.3, чтобы поймать мелкие объекты
            ...
            for box in yolo_results.boxes:
                ...
                # 1. Ловим алкоголь (прямое совпадение)
                ...
                # 2. Ловим ампулы / сигареты (маппинг похожих по геометрии мелких объектов)
    ...
    # --- РАСШИРЕННЫЙ СЛОВАРЬ ДЛЯ АУДИОАНАЛИЗА (Whisper) ---
    try:
        audio_results = whisper_model.transcribe(video_path, language="ru")

        # Ключевые слова, разбитые по подклассам
        ...
        for segment in audio_results.get("segments", []):
            ...
            # Проверка на курение в разговоре
            ...
            # Проверка на упоминание алкоголя
